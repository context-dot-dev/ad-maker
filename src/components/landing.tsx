            ...
            <form onSubmit={(e) => { e.preventDefault(); if (url.trim()) void loadBrand(url); }} className="mt-8 w-full max-w-lg animate-fade-up [animation-delay:180ms]">
              <div className="flex flex-col gap-2 rounded-2xl bg-card p-1.5 shadow-[0_8px_60px_-16px_rgba(112,0,255,0.55)] ring-1 ring-border backdrop-blur focus-within:ring-2 focus-within:ring-primary sm:flex-row sm:items-center">
                <div className="flex min-w-0 flex-1 items-center gap-2">
                  ...
                  <input type="text" value={url} onChange={(e) => setUrl(e.target.value)} placeholder="Enter your brand URL, e.g. stripe.com" className="min-w-0 flex-1 bg-transparent px-1 py-2 text-base text-foreground placeholder:text-muted-foreground focus:outline-none sm:text-sm" disabled={loadingBrand} />
                </div>
                <button type="submit" disabled={loadingBrand || !url.trim()} className="btn-gradient w-full shrink-0 whitespace-nowrap sm:w-auto">
